...
class Peptide(object):
	...
	def init_peptide(cls, peptide_input):
		if peptide_input.endswith(".pdb"):
			...
			return Peptide.frompdb(peptide_input)
		else:
	...
	def get_peptide_templates(self, receptor_allotype, anchors, max_no_templates, similarity_threshold, use_motifs, cv=''):
		...
		# ROMANOS DID NOT FINISH THIS.....
		# original code does NOT work with sequence input or redocking...
		...
		for seq_record in SeqIO.parse("./helper_files/Proper_files/template_sequences.fasta", "fasta"):
			...
			alignments = pairwise2.align.globalxx(receptor_allotype, str(seq_record.seq))
			if(alignments[0].score == best_score): # The second condition is for not fetching alleles that we already have the structure for (mostly for cv purposes)
		...
		print('pretending that allele is ',best_record_list[0])
		receptor_allotype = best_record_list[0]
		...
		similarity_matrix = similarity_matrix[similarity_matrix["Allele"] == receptor_allotype]
		similarity_matrix = similarity_matrix.drop("Allele", axis=1).T.reset_index(drop=False)
		######
		# print('69'*25)
		# print(receptor_allotype)
	...
	def prepare_for_scoring(self, filestore):
		...
		clean = "lps"
		print("python " + prep_peptide_loc + " -l " + self.pdb_filename + " -o " + self.pdbqt_filename + " -A None -Z -U " + clean + " -g -s > " + filestore + "/04_pdbqt_peptides/prepare_ligand4.log 2>&1")
		call(["python " + prep_peptide_loc + " -l " + self.pdb_filename + " -o " + self.pdbqt_filename + " -A None -Z -U " + clean + " -g -s > " + filestore + "/04_pdbqt_peptides/prepare_ligand4.log 2>&1"], shell=True)
		...
		try:
			...
			print('File not found: ',self.pdbqt_filename)
			with open(filestore + "/05_per_peptide_results/peptide_" + str(self.index) + ".log", 'w') as peptide_handler:
