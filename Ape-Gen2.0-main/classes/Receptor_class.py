...
def model_receptor(allele_sequence, peptide_sequence, allotype, filestore, cv):
	...
	B2M = '' # TODO: Fix this
	os.chdir('../../../tfold')
	with open('MHC'+ran_key+'.fasta','w') as f:
		f.write('>M\n'+allele_sequence+'\n')
		f.write('>N\n'+B2M+'\n')
		f.write('>P\n'+peptide_sequence+'\n')
		# python projects/tfold_tcr/predict.py --json examples/pmhc_example.json --output examples/predictions/ --model_version pMHC
	os.system('python3 projects/tfold_tcr/predict.py --fasta MHC'+ran_key+'.fasta --output output/'+filestore+'MHC.pdb --model_version pMHC')
...
class Receptor(object):
	...
	def fromallotype(cls, allotype, peptide_sequence, filestore, cv=''):
		...
		## We no longer care... ai models all
		...
		if len(allotype) > 50:
			print('allotype is a sequence!!!')
			pdb_filename, new_allotype = model_receptor(allotype, peptide_sequence, "NA", filestore, cv)
			return(cls(allotype=allotype, pdb_filename=pdb_filename))
